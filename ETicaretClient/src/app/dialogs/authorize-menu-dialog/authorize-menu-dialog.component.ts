...
export class AuthorizeMenuDialogComponent extends BaseDialog<AuthorizeMenuDialogComponent> implements OnInit {
  ...
  async assignRoles(_roles : MatSelectionList){
    const roles : string[]= _roles.selectedOptions.selected.map(o=>o._elementRef.nativeElement.innerText)
    this.spinnner.show(SpinnerType.BallAtom);
    await this.authorizationEndpointService.assignRoleEndpoint(roles, this.data.code, this.data.menuName,()=>{this.spinnner.hide(SpinnerType.BallAtom);}, error=>{})
  }
