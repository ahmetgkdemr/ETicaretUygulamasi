...
export class ListComponent extends BaseComponent implements OnInit{
  ...
  async getUsers(){
    ...
    const all_users:{totalUsersCount:number; users: List_User[]}= await this.userService.getAllUsers(this.paginator? this.paginator.pageIndex:0,
    ...
    this.dataSource=new MatTableDataSource<List_User>(all_users.users)
    this.paginator.length=all_users.totalUsersCount;
  }
  ...
  this.dialogService.openDialog({
    ...
    afterClosed: ()=>{
      this.alertify.message("Roller başarıyla atanmıştır.",{
        messageType: MessageType.Success,position:Position.TopRight
      })
    }
