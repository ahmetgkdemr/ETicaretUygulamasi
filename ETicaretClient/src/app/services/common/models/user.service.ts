...
export class UserService {
  ...
  async updatePassword(userId: string, resetToken: string, password: string, passwordConfirm : string , successCallBack?: ()=>void, errorCallBack?:(error)=>void): Promise<void>{  
  ...
  async assignRoleToUser(id: string, roles : string[], successCallBack? : ()=>void, errorCallBack?: (error)=>void){
    ...
    const promiseData=firstValueFrom(observable)
    promiseData.then(()=>successCallBack()).catch(error=>errorCallBack(error));
    await promiseData; //burada geriye bir şey döndürmeye gerek yok çünkü bir get işlemi yok bizim için backende isteği yapsın veri tabanı güncellensin yeterlidir.
  }
